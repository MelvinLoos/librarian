...
export class ProgressController {
  ...
  async updateProgress(
    ...
    this.logger.log(
      `Received request to update progress for user: ${userId}, book: ${bookId} to locator ${dto.locator}, percentage ${dto.percentage}`,
    );
  ...
  @ApiOperation({
    ...
    description:
      'Retrieves all books currently being read by the authenticated user, sorted by recent activity.',
  })
  @ApiResponse({
    status: 200,
    description: 'Reading states retrieved successfully.',
  })
  @ApiResponse({ status: 401, description: 'Unauthorized.' })
  async getStates(@Request() req: RequestWithUser) {
    const userId = req.user.id;
    this.logger.log(
